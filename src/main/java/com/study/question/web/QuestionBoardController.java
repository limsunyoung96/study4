...
public class QuestionBoardController {
	...
	public String questionList(@ModelAttribute("searchVO") QuestionBoardSearchVO searchVO
	...
	public String questionView(@RequestParam(value = "boNo") int no 
		  , @RequestParam(value = "mode" , required = false, defaultValue = "sunyoung") String mode
		  , ModelMap model) throws Exception {
		try {		
	...
	@RequestMapping(path = "/question/questionRegist.wow"
			       , method = RequestMethod.POST )
	public ModelAndView questionRegist(@ModelAttribute("boardVO") @Validated({Default.class, RegistType.class}) QuestionBoardVO board
		...
		try {
			...
			message.setResult(false).setTitle("글 등록 실패").setMessage("해당 글번호가 존재합니다.").setUrl("/question/questionList.wow")
					.setUrlTitle("목록으로");
			// 속성에 messageVO 로 저장
	...
	public String questionModify(@ModelAttribute("boardVO") @Validated({Default.class, ModifyType.class}) QuestionBoardVO board
								, BindingResult errors
								, ModelMap model) throws Exception {
		logger.debug("board={}", board);
		List<CodeVO> categoryList = codeService.getCodeListByParent("BC00");
